/**
 * @name NaoSeEsconda
 * @author Baseado em FriendNotifications de mwittrien (https://github.com/mwittrien/BetterDiscordAddons)
 * @author Modificado por sadkkjkjk
 * @version 2.0.8
 * @description Plugin modificado para mostrar notificações de amigos online / offline / call, compatível com Discord 2024+
 */
...
onStart() {
    try {
        ...
        if (!this.settings.users) this.settings.users = {};
        if (!this.settings.users.observed) this.settings.users.observed = {};
    } catch (e) {
        console.warn("[FriendNotifications] erro no onStart:", e);
    }
    this.forceUpdateAll();
}
			...
			getSettingsPanel (collapseStates = {}) {
				...
				return settingsPanel = BDFDB.PluginUtils.createSettingsPanel(this, {
					...
					children: _ => {
						...
						let observed = this.getObservedData();
						let RelationshipStore = BdApi.findModule(m => m?.getFriendIDs && m?.isFriend);
				if (!RelationshipStore) console.warn("[FriendNotifications] RelationshipStore não encontrado no BdApi.findModuleByProps.");
				let friendIds = [];
				if (RelationshipStore) {
    				friendIds = RelationshipStore.getFriendIDs();
			} else {
    				console.warn("[FriendNotifications] RelationshipStore não encontrado (getFriendIDs).");
		}
    ...
    let RelationshipStore = BdApi.findModule(m => m?.getFriendIDs && m?.isFriend);
    if (!RelationshipStore) {
        console.warn("[FriendNotifications] RelationshipStore não encontrado no getOnlineCount.");
        return 0;
    }
    ...
    return friendIds.filter(id =>
        BDFDB.LibraryStores.PresenceStore.getStatus(id) != BDFDB.LibraryComponents.StatusComponents.Types.OFFLINE
    ).length;
}
